...
def env_worker_robotwin(task_name, _, trial_id, trial_seed, config, input_queue, output_queue, is_valid, __, ___):    
    try:
        print("enter env_worker_robotwin", flush=True)
        if multiprocessing.get_start_method(allow_none=True) != 'spawn':
            print("multiprocessing not spawn", flush=True)
        else:
            print("multiprocessing is spawn", flush=True)
        success = False
...
class RobHFRollout(BaseRollout):
    ...
    def _generate_minibatch(self, prompts):
        ...
        for idx in range(batch_size):
            ...
            #input_q = Queue()
            #output_q = Queue()
            print("****ready to start Process****", flush=True)
